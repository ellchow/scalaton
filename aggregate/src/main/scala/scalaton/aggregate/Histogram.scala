...
trait HistogramModule{
  ...
  //   def averageTarget(h: HistogramData[(A, Y), (Long, Y)], p: Double): Option[Y] = bucketsFor(h, p) map {
  ...
  // abstract class Histogram[A, B](implicit mon: Monoid[B], hv: HistogramValue[B], hp: HistogramPoint[A,B]){
  //   /** empty histogram **/
  //   val empty: HistogramData[A,B] = Tag(HistogramDataLTE1[A,B](TreeMap[Double,B](), Double.PositiveInfinity, Double.NegativeInfinity))

  //   def apply(as: A*) = insert(empty, as)

  //   /** gap size for deciding which buckets are closest (to be merged) **/
  //   def gapSize(x: (Double, Long), y: (Double, Long)): Double

  //   /** insert a point into the histogram **/
  //   def insert(h: HistogramData[A,B], a: A): HistogramData[A,B] = {
  ...
  //   /** insert a point into the histogram n times **/
  //   def insertN(h: HistogramData[A,B], a: A, n: Int): HistogramData[A,B] = {
  //     require(n gt 0)

  //     (1 to n).foldLeft(h){ case (hh, _) => insert(hh, a) }
  //   }

  //   def insert(h: HistogramData[A,B], as: Iterable[A]): HistogramData[A,B] =
  //     as.foldLeft(h){ case (hh, a) => insert(hh, a) }

  //   def insert(h: HistogramData[A,B], as: A*): HistogramData[A,B] =
  //     insert(h, as)

  //   /** merge 2 histograms together **/
  //   def merge(h1: HistogramData[A,B], h2: HistogramData[A,B]): HistogramData[A,B] = {
  //     require(h1.)
  ...
  //   /** get buckets (in ascending order) from histogram up to buckets that contain counts for the point **/
  //   def upTo(h: HistogramData[A,B], p: Double): Option[List[(Double, B)]] = {
  ...
  //   def bucketsFor(h: HistogramData[A,B], p: Double): Option[((Double, B), (Double, B))] =
  //     upTo(h,p) flatMap {
  //       _.takeRight(2) match {
  //         case (a :: b :: Nil) => (a, b).some
  //         case _ => none
  ...
  //   /** sum of counts from -Inf to p **/
  //   def cumsum(h: HistogramData[A,B], p: Double): Option[Double] = {
  //     @annotation.tailrec
  //     def loop(bs: List[(Double, Double)], total: Double): Double = bs match {
  //       case (x0, y0) :: (x1, y1) :: Nil =>
  //         val y = y0 + (y1 - y0) / (x1 - x0) * (p - x0)
  ...
  //   /** find quantile using binary search **/
  //   def quantile(h: HistogramData[A,B], q0: Double, tol: Double = 0.001): Option[Double] = {
  //     require((q0 gte 0.0) && (q0 lte 1.0))
  ...
  //   def quantiles(h: HistogramData[A,B], qs: List[Double], tol: Double = 0.001): Option[List[(Double, Double)]] =
  ...
  //   def insertN[A, B, T](h: HistogramData[A,B], a: A, n: Int)
  //                       (implicit hv: HistogramValue[B], mon: Monoid[B], hp: HistogramPoint[A, B], hst: Histogram[A, B, T]): HistogramData[A,B] =
  //     hst.insertN(h, a, n)

  //   def insert[A, B, T](h: HistogramData[A,B], as: A*)
  //                      (implicit hv: HistogramValue[B], mon: Monoid[B], hp: HistogramPoint[A, B], hst: Histogram[A, B, T]): HistogramData[A,B] =
  //     hst.insert(h, as)

  //   def merge[A, B, T](h1: HistogramData[A,B], h2: HistogramData[A,B])
  //                     (implicit hv: HistogramValue[B], mon: Monoid[B], hp: HistogramPoint[A, B], hst: Histogram[A, B, T]): HistogramData[A,B] =
  //     hst.merge(h1, h2)

  //   def cumsum[A, B, T](h: HistogramData[A,B], p: Double)
  //                      (implicit hv: HistogramValue[B], mon: Monoid[B], hp: HistogramPoint[A, B], hst: Histogram[A, B, T]): Option[Double] =
  //     hst.cumsum(h, p)

  //   def quantile[A,B,T](h: HistogramData[A,B], q0: Double, tol: Double = 0.001)
  //                      (implicit hv: HistogramValue[B], mon: Monoid[B], hp: HistogramPoint[A, B], hst: Histogram[A, B, T]): Option[Double] =
  //     hst.quantile(h, q0, tol)

  //   def quantiles[A, B, T](h: HistogramData[A,B], qs: List[Double], tol: Double = 0.001)
  ...
  //   def averageTarget[A, Y, T](h: HistogramData[(A, Y), (Long, Y)], p: Double)
