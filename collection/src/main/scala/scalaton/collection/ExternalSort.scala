...
object ExternalSort {
  ...
  def sortBy[A : EncodeJson : DecodeJson, K : Ordering](xs: Process[Task,A], chunkSize: Int, tmp: Path)(key: A => K): Process[Task,A] = {
    lazy val paths = {
      xs.chunk(chunkSize).zipWithIndex.flatMap{ case (chunk, i) =>
    ...
    io.resource(Task.delay(\/.fromTryCatch(sort)))(_ =>
      Task.delay{ inps.foreach{ i => i.close } ; Filesystem.delete(tmp, true) })(
      disj => Task.delay{ disj.fold(e => throw e , s => if (s.hasNext) s.next else throw Process.End )}
    )
  }
}
